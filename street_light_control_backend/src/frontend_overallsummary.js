import { getMergedLdDetailsJson } from './util.js';
// Helper function to get overall summary data
export async function getOverallSummary(conn, pool) {
  ...
  // Use the correct pool object for getMergedLdDetailsJson
  const merged_ld_details_json = await getMergedLdDetailsJson(pool);
  console.log('Merged LD Details JSON:', merged_ld_details_json);
...
export function setupFrontendOverallSummary(app, pool) {
  ...
  app.get('/api/overall-summary', async (req, res) => {
    ...
    try {
      ...
      const responseData = await getOverallSummary(conn, pool);
